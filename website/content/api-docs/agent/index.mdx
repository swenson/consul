...
- `address` `(string: <required>)` - Specifies the address of the other agent to
  ...
  [can be URL-encoded](/api-docs#url-encoded-resource-names).
...
- `node_name` `(string: <required>)` - Specifies the name of the node to be forced into `left` state.
  This parameter can be [URL-encoded to pass values with URL-invalid characters](/api-docs#url-encoded-resource-names).
