...
const DNSName = "`^([a-zA-Z0-9_]{1}[a-zA-Z0-9_-]{0,62}){1}(\\.[a-zA-Z0-9_]{1}[a-zA-Z0-9_-]{0,62})*[\\._]?$`"
...
func (e *TerminatingGatewayConfigEntry) Validate() error {
	...
	for _, endpoint := range e.Endpoints {
		...
		if err := validateLinkedEndpointAddress(endpoint.Address); err != nil {
...
func validateLinkedEndpointAddress(address string) error {
	var valid bool
	...
	// Hostname
	re := regexp.MustCompile(DNSName)
	valid = valid || re.MatchString(address)

	if !valid {
		return fmt.Errorf("cloud not validate address %s as an IP, CIDR block or Hostname", address)
	}
	return nil
}
